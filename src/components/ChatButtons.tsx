...
/**
 * WhatsApp + Facebook Messenger CTA buttons, used across multiple pages.
 */
export function ChatButtons({ className = "", size = "base" }: ChatButtonsProps) {
  const px = size === "sm" ? "px-5 py-2.5" : "px-7 py-3";
  ...
  return (
    <div className={`flex flex-wrap items-center justify-center gap-3 ${className}`}>
      ...
      <a
        ...
        className={`inline-flex items-center gap-2 rounded-full font-semibold transition ${px} ${text}`}
      ...
      <a
        ...
        rel="noopener noreferrer"
        className={`inline-flex items-center gap-2 rounded-full font-semibold transition ${px} ${text}`}
        style={{ background: "linear-gradient(135deg,#0084ff,#a033ff)", color: "#fff", boxShadow: "0 4px 14px rgba(0,132,255,0.3)" }}
      >
        <IconMessenger />
        Messenger
      </a>
